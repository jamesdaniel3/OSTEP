...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  if (a == 0){
    a = PGSIZE;    // do not allow for allocating the first page
  }
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = PGSIZE; i < sz; i += PGSIZE){
    ...
    // rather than making every page user and writable, copy the results from the parent 
    int perms = *pte & (PTE_W | PTE_U);
    if(mappages(d, (void*)i, PGSIZE, PADDR(mem), perms) < 0)
      goto bad;
  }
  return d;
...
{
  // is the pointer page aligned 
  if ((int) addr % PGSIZE != 0) {
    return -1;
  }
  // is the len greater than 0
  if (len < 1) {
    return -1;
  }
  // are all of the requested pages initialized
  for (int i =0; i < len; i++){
    pte_t * page = walkpgdir(proc->pgdir, addr + (i * PGSIZE), 0);
    if (!page) {
      return -1;
    }
    // update permissions
